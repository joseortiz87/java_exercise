 ...
 */
package com.company.lib.urlbuilder.util;
...
public class Encoder {
	...
	protected String urlEncode(final String input, final boolean isPath, final boolean isFragment,
			final boolean isUserInfo) {
		final StringBuilder sb = new StringBuilder();
		...
		for (int i = 0; i < Character.codePointCount(inputChars, 0, inputChars.length); i++) {
			...
			if (Character.isBmpCodePoint(codePoint)) {
				...
				if ((isPath && Rfc3986Util.isPChar(c) && c != '+') || isFragment && Rfc3986Util.isFragmentSafe(c)
						|| isUserInfo && c == ':' || Rfc3986Util.isUnreserved(c)) {
					sb.append(c);
					continue;
				} else {
			...
			for (int j = 0; j < bb.limit(); j++) {
				// Until someone has a real problem with the performance of this
				// bit,
				// I will leave this less optimal, but much simpler
